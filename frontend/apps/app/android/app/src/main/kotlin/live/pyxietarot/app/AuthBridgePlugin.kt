...
/** Mirrors the web app's access token into native storage so the widget's background worker can
 * authenticate without the WebView running; also exposes an explicit refresh trigger for app-side
 * events (e.g. a new diary entry) that don't otherwise touch the token. */
